	...
	useEffect(() => {
		const fetchUserCurrentLoansCount = async () => {
			if (authState && authState.isAuthenticated) {
				...
				const currentLoansCountResponses = await fetch(url, requestOptions);

				if (!currentLoansCountResponses.ok) {
					throw new Error("Something went Wrong!");
				}

				const currentLoansCountResponseJson = await currentLoansCountResponses.json();
			...
			<div className="container d-none d-lg-block">
				{displayError && (
					<div
						...
						role="alert">
						Please clear dues / late returns
					</div>
				)}
