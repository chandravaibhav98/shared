...
import com.chandravaibhav98.jobportal.Entity.RecruiterJobsInterface;
...
public interface JobPostActivityRepository extends JpaRepository< JobPostActivity, Integer > {
	...
	@Query ( value = " SELECT COUNT(s.user_id) as totalCandidates,j.job_post_id,j.job_title,l.id as locationId,l.city,l.state,l.country," +
					 "c" + ".id as companyId,c.name FROM job_post_activity j " + " inner join job_location l " +
					 " on j.job_location_id = l.id" + " " + " INNER join job_company c  " + " on j.job_company_id = c.id " +
	...
	List< RecruiterJobsInterface > getRecruiterJobs ( @Param ( "recruiter" ) int recruiter );
	...
	List< JobPostActivity > searchWithoutDate (
			...
			@Param ( "location" ) String location , @Param ( "remote" ) List< String > remote , @Param ( "type" ) List< String > type );
